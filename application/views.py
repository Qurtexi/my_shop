...
class ProductDetailView(CartMixin, views.View):

    def get(self, request, *args, **kwargs):
        prod = self.kwargs['product_slug']
        categories = Category.objects.all().order_by('-id')[:5]
        product = Product.objects.get(slug=prod)
        ...
        return render(request, 'detail/product_detail.html', context)


class CategoryDetailView(CartMixin, views.View):

    def get(self, request, *args, **kwargs):
        category = self.kwargs['slug']
        products = Product.objects.filter(category__slug=category).all()[:6]
        categories = Category.objects.all().order_by('-id')[:5]
        c_name = Category.objects.get(slug=category)
        ...
        return render(request, 'detail/category_detail.html', context)
